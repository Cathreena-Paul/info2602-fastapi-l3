import typer
from app.database import get_session, drop_all, create_db_and_tables
...
def list_all_todos():
    with get_session() as db: # Get a connection to the database
        ...
        for todo in todos:
            User = db.exec(select(User).where(User.id == todo.user_id)).first()
            print(f"Todo: {todo.id}, User: {User.username}, Text: {todo.text}, Done: {todo.done}")
